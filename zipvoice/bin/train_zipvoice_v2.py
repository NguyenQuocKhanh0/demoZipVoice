...
from zipvoice.dataset.datamodule_v2 import TtsDataModule
from zipvoice.models.zipvoice_v2 import ZipVoiceTokenTTS
...
def compute_token_loss(
    ...
    info = MetricsTracker()
    num_frames = audio_tokens_lens.sum().item()
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        tokens, audio_tokens, features_lens = prepare_input(
            params=params,
            batch=batch,
            device=device,
            return_tokens=True,
            return_audio_tokens=True,   # <-- bạn phải implement

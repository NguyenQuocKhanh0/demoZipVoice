...
class ZipVoice(nn.Module):
    ...
    def forward_text_inference_ratio_duration(
        ...
        prompt_space_lens = torch.tensor(
            [(score_tokens(token)-(token.count(3) - numsp - 1))*100 for token, numsp in zip(prompt_tokens,num_space_prompt)],
            dtype=torch.int64,
            device=device,
        )
        tokens_space_lens = torch.tensor(
            [(score_tokens(token)-(token.count(3) - numsp - 1))*100 for token, numsp in zip (tokens,num_space_text)],
            dtype=torch.int64,
            device=device,
        )

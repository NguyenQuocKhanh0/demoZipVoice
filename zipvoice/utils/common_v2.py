...
def prepare_input(
    ...
    # token path (mới)
    if return_audio_tokens:
        audio_tokens = batch["audio_tokens"].to(device).long()            # [B,T,16]
        audio_tokens_lens = batch["audio_tokens_lens"].to(device).long()  # [B]
        return_list += [audio_tokens, audio_tokens_lens]

...
from zipvoice.dataset.token_wrapper import AudioTokenWrapper
import argparse
import logging
...
class TtsDataModule:
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            "--audio-token-dir",
            type=Path,
            default=Path("data/qwen_tokens_pt"),
            help="Directory chứa {cut_id}.pt (tensor [T,16])",
        )
        group.add_argument(
            "--use-audio-tokens",
            type=str2bool,
    ...
    def train_dataloaders(
        ...
        if self.args.use_audio_tokens:
            train = AudioTokenWrapper(train, token_dir=self.args.audio_token_dir)
